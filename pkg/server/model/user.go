...
func InsertUser(record *User) error {
	// TODO→DONE: usersテーブルへのレコードの登録を行うSQLを入力する
	// 変動値が存在するため、Prepare を使う
	prep, err := db.Conn.Prepare("INSERT INTO `user`(`id`, `auth_token`, `name`, `high_score`, `coin`) VALUES (?,?,?,?,?);")
	if err != nil {
		return err
	}
	_, err = prep.Exec(record.ID, record.AuthToken, record.Name, record.HighScore, record.Coin)
	return err
}

// SelectUserByAuthToken auth_tokenを条件にレコードを取得する
func SelectUserByAuthToken(authToken string) (*User, error) {
	// TODO→DONE: auth_tokenを条件にSELECTを行うSQLを第1引数に入力する
...
func UpdateUserByPrimaryKey(record *User) error {
	// TODO→不安: idを条件に指定した値でnameカラムの値を更新するSQLを入力する
	prep, err := db.Conn.Prepare("UPDATE `user` SET `name`=?, WHERE `id`=?")
	if err != nil {
		return err
	}
	_, err = prep.Exec(record.Name, record.ID)
	return err
}
